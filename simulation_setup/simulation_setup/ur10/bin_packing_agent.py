...
from omni.isaac.cortex.sample_behaviors.ur10.policy import Policy
from omni.isaac.cortex.sample_behaviors.ur10.value import Value
    ...
    def validate(self, rollouts_no = 1000, single_rollout_max_length = 120):
        ...
        for rollout in tqdm (range(rollouts_no), desc="Loading..."):
            ...
            for t in range(single_rollout_max_length):
               ...
               action = torch.vstack([self.agent.act(observation[0:2], timestep=t, timesteps=single_rollout_max_length)[0]])
               observation, reward, terminated, truncated, info = self.env.step(action)
               if terminated:
                   break
            area_utilized.append(self.env.get_area_used())
            space_utilized.append(self.env.get_volume_used())
            #illegal_actions.append(self.env.illegal_actions)

        print("The area utilzied is: {}%.".format(statistics.mean(area_utilized)))
        print("The area utilization mode is: {}%.".format(statistics.mode(area_utilized)))
        ...
        self.area_utilizations = area_utilized
        self.space_utilizations = space_utilized

    def get_confidence_interval_area(self, conf_level = 0.95, n_bootstrap = 80000):
        mean = statistics.mean(self.area_utilizations)
        bootstrap_sampels = []

        for i in range(n_bootstrap):
            sample = np.random.choice(self.area_utilizations, size = len(self.area_utilizations), replace = True)
            bootstrap_sampels.append(sample)
        means = [np.mean(s) for s in bootstrap_sampels]

        num_bins = 200
        hist, bins = np.histogram(self.area_utilizations, bins=num_bins)
        
        # Plot the histogram as a bar plot
        plt.bar(bins[:-1], hist, width=(bins[1] - bins[0]), alpha=0.8, color = "#f700f8", label = "Freqency")

        ### Fit a normal distribution to the data
        mu, std = norm.fit(self.area_utilizations)
        
        # Plot the PDF over the histogram
        xmin, xmax = plt.xlim()
        x = np.linspace(xmin, xmax, 2000)
        p = norm.pdf(x, mu, std) * len(self.area_utilizations) * (bins[1] - bins[0])  # Scaling PDF to match histogram
       ...
       # Add vertical dashed lines for lower and upper bounds
        pdf_lower_bound = norm.pdf(lower_bound, mu, std) * len(self.area_utilizations) * (bins[1] - bins[0])
        pdf_upper_bound = norm.pdf(upper_bound, mu, std) * len(self.area_utilizations) * (bins[1] - bins[0])
