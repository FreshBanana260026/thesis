...
class PackingEnvNp(gym.Env):
    ...
    def __init__(self, render_mode = None, size = 40):
        ...
        self.rotating = False
        self.ignore = 4
        self.bin_surface = np.zeros((self.size, self.size))
        self.observation_space = spaces.Dict(
            {
                "observation": spaces.Box(0, self.z - 1, shape=(self.x,self.y), dtype=np.float64),
                "box": spaces.Box(0, self.box_size - 1, shape=(3,), dtype=np.float64),
                "boxes": spaces.Box(0, self.box_size - 1, shape=(self.no_items, 3), dtype=np.float64),
            }
        )
    ...
    def get_area_used(self):
        return np.count_nonzero(self.bin_surface)/16 #in %
    ...
    #this fn calculates are of package that is going to be supported after placing it inside the bin
    ...
    #this function has only visualization purposes
    def _render_frame(self):
        ...
        for index_tuple in zip(nonzero_indices[0], nonzero_indices[1]):
            ...
            font = pygame.font.Font(None, 24)  # You can adjust the font size
        ...
        if self.render_mode == "human":
            # The following line copies the drawings from `canvas` to the visible window
